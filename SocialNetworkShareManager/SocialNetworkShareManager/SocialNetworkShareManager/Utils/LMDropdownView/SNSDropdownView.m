...
@implementation SNSDropdownView

#pragma mark - INIT

+ (instancetype)dropdownView
{
    return [[SNSDropdownView alloc] init];
}
...
- (id)init
{
    ...
    if (self) {
        ...
        _direction = SNSDropdownViewDirectionTop;
        _currentState = SNSDropdownViewStateDidClose;
...
- (BOOL)isOpen
{
    return (_currentState == SNSDropdownViewStateDidOpen);
}

- (void)showInView:(UIView *)containerView withContentView:(UIView *)contentView atOrigin:(CGPoint)origin
{
    if (_currentState != SNSDropdownViewStateDidClose) {
        return;
    }
    
    // Start showing
    _currentState = SNSDropdownViewStateWillOpen;
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(self.animationDuration * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        _currentState = SNSDropdownViewStateDidOpen;
...
- (void)hide
{
    if (_currentState != SNSDropdownViewStateDidOpen) {
        return;
    }
    
    // Start hiding
    _currentState = SNSDropdownViewStateWillClose;
...
- (void)setupContentView:(UIView *)contentView inView:(UIView *)containerView atOrigin:(CGPoint)origin
{
    ...
    UIImage *capturedImage = [UIImage sns_imageFromView:containerView withSize:capturedSize];
    UIImage *blurredCapturedImage = [capturedImage sns_blurredImageWithRadius:self.blurRadius iterations:5 tintColor:[UIColor clearColor]];
    ...
    switch (self.direction) {
        case SNSDropdownViewDirectionTop:
            ...
            break;
        case SNSDropdownViewDirectionBottom:
            contentView.frame = CGRectMake(0,
                                           0,
                                           CGRectGetWidth(contentView.frame),
                                           CGRectGetHeight(contentView.frame));
...
- (void)addContentAnimationForState:(SNSDropdownViewState)state
{
...
- (void)addContainerAnimationForState:(SNSDropdownViewState)state
{
...
- (NSArray *)contentPositionValuesForState:(SNSDropdownViewState)state
{
    ...
    if (state == SNSDropdownViewStateWillOpen || state == SNSDropdownViewStateDidOpen)
    {
        switch (self.direction) {
            case SNSDropdownViewDirectionTop:
...
- (NSArray *)contentKeyTimesForState:(SNSDropdownViewState)state
{
...
- (NSArray *)contentTimingFunctionsForState:(SNSDropdownViewState)state
{
...
- (NSArray *)containerTransformValuesForState:(SNSDropdownViewState)state
{
...
- (NSArray *)containerKeyTimesForState:(SNSDropdownViewState)state
{
    NSMutableArray *keyTimes = [[NSMutableArray alloc] init];
    [keyTimes addObject:[NSNumber numberWithFloat:0]];
    [keyTimes addObject:[NSNumber numberWithFloat:0.5]];
...
- (NSArray *)containerTimingFunctionsForState:(SNSDropdownViewState)state
{
    NSMutableArray *timingFunctions = [[NSMutableArray alloc] init];
    [timingFunctions addObject:[CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseOut]];
    [timingFunctions addObject:[CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseInEaseOut]];
